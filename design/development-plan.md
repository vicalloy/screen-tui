...
| **M1** | P0 核心闭环 | 手机尺寸模拟下走通「看 → 选 → 进 → 出」；detach 必回 TUI；`kill -9` 后终端不报废 | 未开始 |
| **M2** | P1 增强 | P0 + P1 全量 FR 验收通过；危险操作均二次确认；预览降级路径可用 | 未开始 |
...
| T2.2 元数据探测 | `screen::probe`：Linux `/proc`、macOS `ps`/`lsof` | T0.3 | 取不到返回 `None`，UI 隐藏字段，不猜测（C-5）；探测失败不影响主流程 |
