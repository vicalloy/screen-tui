...
| T2.4 会话操作 | 远程断开 `D`、终止 `K`（确认框默认焦点在取消）、重命名 `r`、dead 清理 `W`（`-wipe` + 确认） | T1.5 | 每个危险操作二次确认；确认框显示会话名+命令；操作前重新校验状态（NFR-08）；attached/multi 走强制终止确认框（红色警示断开对端，确认后照杀）、自身所在会话（`$STY`）拒 kill（FR-13 验收 5/6） |
