...
pub struct Dict {
    ...
    pub kill_self: &'static str,
    pub confirm_kill_force: &'static str,
...
const DICT_EN: Dict = Dict {
    ...
    kill_attached: "'{0}' became attached since confirmation; kill refused — press K again to force kill",
    kill_self: "you are inside '{0}'; killing the session you are in is refused",
    confirm_kill_force: "WARNING: attached — force kill will disconnect the other terminal",
...
const DICT_ZH: Dict = Dict {
    ...
    kill_attached: "'{0}' 确认后已转为连接状态；拒绝终止 —— 如需强制终止请再按 K",
    kill_self: "你正在 '{0}' 会话内；拒绝终止自身所在的会话",
    confirm_kill_force: "警告：该会话正被连接 —— 强制终止会把对端终端一起断开",
